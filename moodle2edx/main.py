...
class Moodle2Edx(object):
    ...
    def __init__(self, infn, edxdir='.', org="UnivX", semester="2014_Spring", verbose=False, clean_up_html=True):
        ...
        os.popen('xmllint -format --encode UTF-8 -o %s/course/%s.xml -' % (cdir, semester), 'w').write(
            etree.tostring(cxml, pretty_print=True))

        # the actual top-level course.xml file is a pointer XML file to the one in course/semester.xml
        open('%s/course.xml' % cdir, 'w').write(
            '<course url_name="%s" org="%s" course="%s"/>\n' % (semester, org, number))
    ...
    def convert_static_files(self):
        ...
        for mfile in fxml.findall('file'):
            ...
            cmd = 'cp %s/files/%s/%s "%s/static/%s"' % (self.moodle_dir, fhash[:2], fhash, self.edxdir, fname2)
    ...
    def new_vertical(self, seq, name, makevert=False):
        # seq = etree.SubElement(chapter,'sequential')
        # self.set_sequential_name(seq, name)
        if makevert:
    ...
    def activity2chapter(self, activity, sections, cxml, seq, vert, qdict):
        ...
        if not sectionid in sections:
            ...
            seq = self.new_sequential(sectionid, chapter, activity_title=title)
            vert = None
        else:
        ...
        if category == 'url':
            if vert is None:  # use current vertical if exists, else create new one
                vert = self.new_vertical(seq, title, makevert=True)
            else:
                print "  ",
            ...
            # seq, vert = self.new_vertical(seq, title)
            ...
            print "  --> book %s (%s)" % (title, adir)
            self.import_book(adir, seq, title)
            vert = None
    ...
    def export_question(self, question, name="", url_name=""):
        ...
        pfn = url_name
        os.popen('xmllint -format --encode UTF-8 -o %s/problem/%s.xml -' % (self.edxdir, pfn), 'w').write(
            etree.tostring(problem, pretty_print=True))
        print "        wrote %s" % pfn
        return None
    ...
    def import_book(self, adir, seq, title):
        ...
        for book_inst in qxml.findall('.//chapter'):
            ...
            if book_inst.find('.//content').text:
                htmlstr = re.sub(' style=".([^"]+)"', '', book_inst.find('.//content').text)
